...
可复用的「软 Schema」设计心法 + 可落地的实现包。用户安装后，Agent 可据此指导其**真正构建**出灵活数据库（个人知识库、碎片收集、表单、多源聚合等）。
...
**Q: 财务报表等金额类数据如何存储？**  
A: 建议**金额统一存「元」**，归档时做单位换算（万元→元、亿元→元）。抽取器输出时转换；查询/展示时再按需格式化。见 [财务报表场景说明](docs/examples/财务报表场景说明.md)。
