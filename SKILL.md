...
description: |
  指导用户设计并实现「灵活数据库」——支持非结构化、多源异构数据的 SQLite 知识库。
  适用场景：个人知识库、PDF/报告知识库、政策信息收集、碎片信息收集、表单/问卷自定义字段、多源数据聚合、事件日志。
  当用户说「想做知识库」「存储 PDF 报告」「报告正文检索」「研报/周报归档」「收集政策/碎片信息」「字段老要改」「多来源数据」时使用本 skill。
  提供：心法、三层模型、Agent 工作流、Schema 模板、参考脚本。
---
...
用户复制本目录到项目或 `.cursor/skills/` 后，Agent 按上述工作流指导落地。
